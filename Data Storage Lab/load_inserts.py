...
DBname = "postgres"
DBuser = "postgres"
DBpwd = "pass"   # insert your postgres db password here
...
def load2(conn, filename):
	with conn.cursor() as cursor:
		print(f"Loading data from {filename} using COPY...")
		start = time.perf_counter()
		...
		elapsed = time.perf_counter() - start
		print(f'Finished loading via COPY. Elapsed Time: {elapsed:0.4f} seconds')
